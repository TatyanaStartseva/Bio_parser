    ...
    try:
        ...
        for user_id in users_ids:
            await bot.send_message(
                user_id, f"Начинаю обработку пользователей с {offset} по {offset+size}"
            )
        pool = db()
        cursor_users = pool["users"]
        cursor = cursor_users.find().sort("dateUpdated", ASCENDING).limit(size)
        ...
        if ban_values:
            cursor_users.update_many(
                {"user_id": {"$in": ban_values}}, {"$set": {"ban": True}}
            )
